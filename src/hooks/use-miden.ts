...
export function useMiden(nodeUrl?: string) {
  ...
  const evmWallets = useMemo(
    () => embedded.wallets?.filter((w) => w.type === "EVM"),
    [embedded.wallets]
  );
  ...
  useEffect(() => {
    ...
    async function setupClient() {
      if (isConnected && evmWallets && para) {
        const { AccountType, AccountId, Address, NetworkId } = await import(
        ...
        const { client: midenParaClient, accountId: aId } =
          await createParaMidenClient(para, evmWallets[0] as Wallet, {
            ...
            nodeTransportUrl: "https://transport.miden.io",
          });

        clientRef.current = midenParaClient;
        setAccountId(
          Address.fromAccountId(AccountId.fromHex(aId), "BasicWallet").toBech32(
            NetworkId.Testnet
          )
        );
      }
